...
- El archivo .rmd con el código utilizado para la limpieza, preparación y visualización del dataset se puede encontrar en el siguiente repositorio de Github: https://github.com/ltmateo/data_visualization_exoplanets 
...
Este mapa interactivo muestra todos los exoplanetas descubiertos hasta ahora. Observa cómo estos mundos se concentran en puntos donde nuestra tecnología nos permite "ver", mientras que existen vastos vacíos que aún permanecen inexplorados.
...
p <- ggplot(data_map, aes(x = ra, y = dec, color = discoverymethod, size = log(pl_rade + 1))) +
  ...
  theme_minimal() +
  labs(title = "Interactive map of discovered exoplanets", 
       x = "Right ascension",
       y = "Declination",
...
ggplot(data_clean, aes(x = discoverymethod, y = pl_rade, fill = discoverymethod, alpha = 0.9)) +
  ...
  scale_y_log10() +
  theme_minimal() +
  labs(title = "Distribution of exoplanet sizes by discovery method",
       x = "Discovery method",
       y = "Planet radius (log)") + 
  scale_fill_manual(values = colors1) +
  theme(legend.position = "none",
